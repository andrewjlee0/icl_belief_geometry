...
For each (family, param, seed, layer):
  ...
  - also score act -> logNTP directly (same split) for the comparison curve
  - also compute the analytic ceiling eta(true) -> logNTP (lin + quad) per seed

All R2s are variance-weighted (pooled SS over output dims).

Outputs: bottleneck_{model}.csv with targets:
  act→log_ntp, etahat→log_ntp, etahat_quad→log_ntp,
  eta→log_ntp (constant over layers), eta_quad→log_ntp (constant over layers)

Defaults are trimmed for speed: main-text params only, 5 seeds.
Use --all_params / --n_seeds 10 for the full sweep.
"""
...
# main-text parametrization labels (must match cfg["label_fn"] output)
MAIN_TEXT = {
    "Mess3":  "a=0.01, x=0.02",
    "Arch":   "a=0.99",
    "Wing":   "a=0.98, x=0.4",
...
def _quad_t(b):
    m = b.shape[1]
    cross = torch.stack([b[:, a] * b[:, c] for a in range(m) for c in range(a, m)], 1)
    return torch.cat([b, cross], 1)

def _lstsq_pred(A_tr, B_tr, A_te):
    A_tr = torch.cat([A_tr, torch.ones(len(A_tr), 1, device=A_tr.device, dtype=A_tr.dtype)], 1)
    A_te = torch.cat([A_te, torch.ones(len(A_te), 1, device=A_te.device, dtype=A_te.dtype)], 1)
    W = torch.linalg.lstsq(A_tr, B_tr).solution
    return A_tr @ W, A_te @ W

def _r2vw(y, p):
    ssr = ((y - p) ** 2).sum()
    sst = ((y - y.mean(0)) ** 2).sum()
    return float(1.0 - (ssr / sst).item())
...
def main():
    ...
    for hmm_name in (args.families or list(HMMS.keys())):
        ...
        if not cfg:
            continue
        params = cfg["params"]
        if not args.all_params and hmm_name in MAIN_TEXT:
        ...
        for param in params:
            ...
            for seed in range(args.n_seeds):
                ...
                yb_tr = torch.tensor(y_beliefs[idx_tr], device=device, dtype=torch.float32)
                yb_te = torch.tensor(y_beliefs[idx_te], device=device, dtype=torch.float32)
                yl_tr = torch.tensor(y_log_ntp[idx_tr], device=device, dtype=torch.float32)
                yl_te = torch.tensor(y_log_ntp[idx_te], device=device, dtype=torch.float32)

                # analytic ceiling: eta(true) -> logNTP (constant over layers)
                _, p_te = _lstsq_pred(yb_tr, yl_tr, yb_te)
                r2_eta_lin = _r2vw(yl_te, p_te)
                _, p_te = _lstsq_pred(_quad_t(yb_tr), yl_tr, _quad_t(yb_te))
                ...
                for l in layers:
                    ...
                    if X.numel() == 0:
                        continue
                    X_tr = X[idx_tr].to(torch.float32); X_te = X[idx_te].to(torch.float32)
                    # direct: act -> logNTP
                    _, p_te = _lstsq_pred(X_tr, yl_tr, X_te)
                    r2_act = _r2vw(yl_te, p_te)
                    # stage 1: probe act -> beliefs; etahat on both splits
                    eh_tr, eh_te = _lstsq_pred(X_tr, yb_tr, X_te)
                    # stage 2: etahat -> logNTP (linear, quadratic)
                    _, p_te = _lstsq_pred(eh_tr, yl_tr, eh_te)
                    r2_eh = _r2vw(yl_te, p_te)
                    _, p_te = _lstsq_pred(_quad_t(eh_tr), yl_tr, _quad_t(eh_te))
                ...
                pbar.update(1)
                pd.DataFrame(all_rows).to_csv(out_path, index=False)   # checkpoint per seed
        pbar.close()
