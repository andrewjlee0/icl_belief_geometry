...
def main():
    ...
    for hmm_name, rep_param in REPRESENTATIVES.items():
        ...
        X = acts[best_layer]
        W = fit_probe(X, torch.tensor(y_true, device=device, dtype=torch.float32), use_bias=True)
        y_pred = predict_probe(X, W, use_bias=True).cpu().numpy()
    ...
    np.savez(os.path.join(args.output_dir, f"geom_{ms}.npz"), **{f"{k}_{v}": geom[k][v] for k in geom for v in ["true","pred"]})
    print("Done.")
